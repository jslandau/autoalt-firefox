...
This section contains the text to paste into the Mozilla Add-ons Developer Hub when creating the AutoAlt listing for Firefox. Most copy mirrors the Chrome Web Store listing; Firefox-specific differences are noted below.
...
AMO uses a different category taxonomy than the Chrome Web Store. Recommended:
...
```
autoalt@sjennings.brokentoys.org
```
...
(Same URL as the Chrome Web Store listing — a single hosted privacy policy covers both browsers, since the data-handling behavior is identical.)
...
AMO has its own permissions review. The justifications are the same as the Chrome Web Store listing (see above), with the same disclosures. The only manifest key Firefox-specific is `browser_specific_settings.gecko`, which is metadata (extension ID + minimum version) and not a permission — it grants no capabilities.
